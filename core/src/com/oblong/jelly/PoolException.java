 ...
 *
 * <p> There are several ways operations on pools can fail. As is
 ...
 * exception classes defined in this package and discriminating
 * between different error types by means of separate catch clauses.
 *
 * <p>Thus, you'll observe that all pool operations throwing
 * exceptions use PoolException in their throws clause (so that a
 * single catch clause will do), but also that all exception types in
 * com.oblong.jelly derive from PoolException, so that you can also
 * write catch clauses separately for them.
 *
 * <p>For example, this snippet uses the first error handling
 * strategy:
 *
 * <pre>
 *   try {
...
public class PoolException extends Exception {
     ...
     * type, which always derives from PoolException.
     */
    public static enum Kind {
        /**
         * Malformed pool or server address.
         * @see BadAddressException
         */
