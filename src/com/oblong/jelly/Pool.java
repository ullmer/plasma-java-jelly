...
/**
 * A collection of functions to operate on pools.
 *
 * All these functions are, strictly speaking, redundant, in the sense
 * that you can perform the same operations via PoolServer instances
 * obtained from the PoolServers factory. But the latter can be a bit
 * roundabout when you don't need to keep the PoolServer around and
 * all you want is a quick way to, say, create a pool or obtain a Hose
 * in one shot. The methods in this class allow you to do precisely
 * that, and to ignore the PoolServer API if you wish to.
 *
 ...
 * in all of them, you can expect any kind (meaning, any
 * PoolException#Kind) of error, with the exception of UNCLASSIFIED
 * and USER, and other obvious cases depending on context (e.g., a
 * call to participate won't throw a POOL_EXISTS error).
 *
...
public final class Pool {
     ...
     *
     * @throws PoolException Possible error kinds include (but are
     * not limited to) PoolException#Kind#POOL_EXISTS (catchable as a
     * com.oblong.jelly.pool.PoolExistsException), and
     * PoolException#Kind#BAD_ADDRESS
     * (com.oblong.jelly.pool.BadAddress), with the obvious meanings.
     */
    ...
    /**
     * Equivalent to Pool.create(addr.toString(), opts).
     */
     ...
     * pool names.
     *
     * Possible error kinds include (but are not limited to)
     * PoolException#Kind#NO_SUCH_POOL (catchable as a
     * com.oblong.jelly.pool.NoSuchPoolException), and
     * PoolException#Kind#BAD_ADDRESS
     * (com.oblong.jelly.pool.BadAddress), with the obvious meanings.
     */
    ...
    /**
     * Equivalent to Pool.dispose(addr.toString()).
     */
    ...
    /**
     * Establishes a connection with the pool denoted by the give URI.
    ...
    /**
     * Equivalent to Pool.participate(addr.toString()).
     */
     ...
     * Pool#participate(String).
     *
     * @throws PoolException No errors of kind
     * PoolException#Kind#POOL_EXISTS or
    ...
    /**
     * Equivalent to Pool.participate(addr.toString(), opts).
     */
    ...
    /**
     * Equivalent to Pool.pools(PoolServerAddress.fromURI(serverURI)).
     */
